...
datadog_keys = json.loads(
    kms.decrypt(CiphertextBlob=base64.b64decode(KMS_ENCRYPTED_KEYS))["Plaintext"]
)
...
def _process_rds_enhanced_monitoring_message(ts, message, account, region):
    ...
    disk_stats = message.get("diskIO", [{}])[
        0
    ]  # we never expect to have more than one disk
    for key, value in disk_stats.items():
        stats.gauge(
            "aws.rds.diskio.%s" % key, value, timestamp=ts, tags=tags, host=host_id
        )
