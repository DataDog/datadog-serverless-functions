...
## @param DD_STORAGE_TAG - boolean - optional -default: true
## Instruct Datadog backend to enrich logs with tags attached to the storage (S3 / LogGroup)
## Ensure Resource Collection in enabled in the AWS Integration
#
DD_STORAGE_TAG = get_env_var("DD_STORAGE_TAG", "true", boolean=True)


DD_FETCH_S3_TAGS = get_env_var("DD_FETCH_S3_TAGS", "true", boolean=True)
DD_FETCH_LOG_GROUP_TAGS = get_env_var("DD_FETCH_LOG_GROUP_TAGS", "false", boolean=True)
DD_FETCH_LAMBDA_TAGS = get_env_var("DD_FETCH_LAMBDA_TAGS", "false", boolean=True)
DD_FETCH_STEP_FUNCTIONS_TAGS = get_env_var(
    "DD_FETCH_STEP_FUNCTIONS_TAGS", "false", boolean=True
)
