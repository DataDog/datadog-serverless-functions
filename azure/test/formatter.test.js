...
var formatter = require('../shared/formatter.js').forTests;
...
describe('Azure Log Monitoring', function() {
  describe('#getLogFormat', function() {
    it('should return string', function() {
        eventHubMessages = '';
        assert.equal(constants.STRING, formatter.getLogFormat(eventHubMessages));
        eventHubMessages = 'foobar';
        assert.equal(constants.STRING, formatter.getLogFormat(eventHubMessages));
    });
    it('should return string array', function() {
      eventHubMessages = ['', 'foobar'];
      assert.equal(constants.STRING_ARRAY, formatter.getLogFormat(eventHubMessages));
    });
    it('should return json with records', function() {
      eventHubMessages = [{'records': [{}, {}]}, {'records': [{}, {}]}];
      assert.equal(constants.JSON_RECORDS, formatter.getLogFormat(eventHubMessages));
    });
    it('should return json object', function() {
      eventHubMessages = {'key': 'value', 'otherkey':'othervalue'};
      assert.equal(constants.JSON_OBJECT, formatter.getLogFormat(eventHubMessages));
    });
    it('should return json no records', function() {
      eventHubMessages = [{'key': 'value', 'otherkey':'othervalue'}, {'key': 'value', 'otherkey':'othervalue'}];
      assert.equal(constants.JSON_ARRAY, formatter.getLogFormat(eventHubMessages));
    });
    it('should return invalid', function() {
      eventHubMessages = 1;
      assert.equal(constants.INVALID, formatter.getLogFormat(eventHubMessages));
      eventHubMessages = () => {};
      assert.equal(constants.INVALID, formatter.getLogFormat(eventHubMessages));
  ...
  describe('#extractResourceId', function() {
    it('should parse a valid record', function() {
      ...
      assert.deepEqual(expectedMetadata, formatter.extractResourceId(record))
    });
