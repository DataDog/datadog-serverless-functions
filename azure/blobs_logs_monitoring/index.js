...
const STRING = 'string';  // blobContent = 'some message'
const STRING_ARRAY = 'string-array';  // blobContent = ['one message', 'two message', ...]
const JSON_OBJECT = 'json-object';  // blobContent = {"key": "value"}
const JSON_RECORDS = 'json-records';  // blobContent = [{"records": [{}, {}, ...]}, {"records": [{}, {}, ...]}, ...]
const JSON_ARRAY = 'json-array';  // blobContent = [{"key": "value"}, {"key": "value"}, ...]
...
module.exports = function (context, blobContent) {
    ...
    logs.forEach(log => {
        var eventHubType = getLogFormat(log);
        switch (eventHubType) {
            case STRING:
                handleLogs(addTagsToStringLog)(log);
                break;
            case JSON_OBJECT:
                handleLogs(addTagsToJsonLog)(log);
                break;
            case STRING_ARRAY:
                log.forEach( handleLogs(addTagsToStringLog) );
                break;
            case JSON_RECORDS:
                log.forEach( message => {
                ...
                break;
            case JSON_ARRAY:
                log.forEach( handleLogs(addTagsToJsonLog) );
                break;
            case INVALID:
            default:
                context.log.warn('log format is invalid');
                break;
        }
    });
    socket.end();
...
function getLogFormat(log) {
    ...
    if (!Array.isArray(log) && typeof log === 'object' && log !== null) {
        return JSON_OBJECT;
    }
